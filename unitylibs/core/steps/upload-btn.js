...
export default async function createUpload(target, callback) {
  ...
  const a = createActionBtn(li);
  const input = createTag('input', { class: 'file-upload', type: 'file', accept: 'image/png,image/jpg,image/jpeg' });
  a.append(input);
  const errorFileSizeText = unityEl.querySelector('.icon-error-filesize').nextSibling.textContent;
  a.addEventListener('change', async (e) => {
    const file = e.target.files[0];
    if (!file) return;
    if (file.size > 1000) {
      unityEl.dispatchEvent(new CustomEvent(errorToastEvent, { detail: {msg: errorFileSizeText} }));
      return;
    }
